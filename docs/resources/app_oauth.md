...
- `consent_method` (String) _Early Access Property_. Indicates whether user consent is required or implicit. Valid values: REQUIRED, TRUSTED. Default value is TRUSTED
...
- `implicit_assignment` (Boolean) _Early Access Property_. Enable Federation Broker Mode.
- `issuer_mode` (String) _Early Access Property_. Indicates whether the Okta Authorization Server uses the original Okta org domain URL or a custom domain URL as the issuer of ID token for this client.
...
- `refresh_token_leeway` (Number) _Early Access Property_ Grace period for token rotation
- `refresh_token_rotation` (String) _Early Access Property_ Refresh token rotation behavior
...
- `wildcard_redirect` (String) _Early Access Property_. Indicates if the client is allowed to use wildcard matching of redirect_uris
